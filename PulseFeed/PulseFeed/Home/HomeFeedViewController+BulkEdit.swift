...
extension HomeFeedViewController {
    ...
    private func setupBulkActionToolbar() {
        ...
        let addTagButton = UIBarButtonItem(
            title: "Tag",
            style: .plain,
            target: self,
            action: #selector(tagSelectedItems)
        )
    ...
    @objc private func tagSelectedItems() {
        guard !selectedItems.isEmpty else { return }
        
        // Create alert controller for tag management
        let alert = UIAlertController(
            title: "Apply Tags",
            message: "Apply tags to \(selectedItems.count) selected items",
        ...
        StorageManager.shared.getTags { [weak self] result in
            ...
            DispatchQueue.main.async {
                ...
                case .success(let tags):
                    ...
                    for tag in tags.sorted(by: { $0.name < $1.name }) {
                        let action = UIAlertAction(
                            ...
                            self?.applyTagToSelectedItems(tag)
                        }
                        alert.addAction(action)
                    }
                    
                    // Add action to create new tag
                    alert.addAction(UIAlertAction(
                        title: "Create New Tag",
                        style: .default
                    ) { [weak self] _ in
                        self?.showCreateTagDialogForSelectedItems()
                    })
                ...
                case .failure:
                    // If we can't get tags, just show create new tag option
                    alert.addAction(UIAlertAction(
                        title: "Create New Tag",
                        style: .default
                    ) { [weak self] _ in
    ...
    private func applyTagToSelectedItems(_ tag: Tag) {
        guard !selectedItems.isEmpty else { return }
        
        // Show loading indicator
        let loadingAlert = UIAlertController(
            title: "Applying Tag",
            message: "Adding tag to \(selectedItems.count) items...",
        ...
        // Apply tag to each selected item
        for index in selectedItems {
            if index < items.count {
                let item = items[index]
                
                group.enter()
                item.addTag(tag) { result in
        ...
        group.notify(queue: .main) { [weak self] in
            ...
            loadingAlert.dismiss(animated: true) {
                // Show result toast
                self.showToast(message: "Tagged \(successCount) items with '\(tag.name)'")
                
                // Reload table to show tags
                self.tableView.reloadData()
            }
        }
    }
    
    private func showCreateTagDialogForSelectedItems() {
        let alert = UIAlertController(
            title: "New Tag",
        ...
        alert.addAction(UIAlertAction(title: "Create", style: .default) { [weak self] _ in
            ...
            // Create the tag
            StorageManager.shared.createTag(name: tagName) { [weak self] result in
                guard let self = self else { return }
                
                switch result {
                case .success(let newTag):
                    // Apply the new tag to selected items
                    DispatchQueue.main.async {
                        self.applyTagToSelectedItems(newTag)
                    }
                case .failure:
                    // Show error message
                    DispatchQueue.main.async {
