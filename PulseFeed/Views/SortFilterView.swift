...
class SortFilterView: UIView {
    ...
    private func showDatePicker(for operation: FilterOperationType) {
        ...
        let today = UIAlertAction(title: "Today", style: .default) { [weak self] _ in
            ...
            self.addFilterRule(field: .dateRange, operation: operation, value: today)
        }
        ...
        if let windowScene = UIApplication.shared.connectedScenes.first as? UIWindowScene,
           let rootViewController = windowScene.windows.first?.rootViewController {
            ...
            doneButton.addAction(UIAction { [weak self, weak vc, weak datePicker] _ in
                ...
                self.addFilterRule(field: .dateRange, operation: operation, value: selectedDate)
                        ...
                        if tags.isEmpty {
                            ...
                            for tag in tags.sorted(by: { $0.name < $1.name }) {
                                let action = UIAlertAction(title: tag.name, style: .default) { [weak self] _ in
                                    self?.addFilterRule(field: .tag, operation: operation, value: tag.id)
                                }
                                alert.addAction(action)
                            }
    ...
    private func toggleFilterCombination() {
        ...
        let newFilterOption = FilterOption(
    ...
    private func updateSortButtonState() {
        ...
        if currentSortOption.field == SortOption.default.field && 
           currentSortOption.order == SortOption.default.order {
